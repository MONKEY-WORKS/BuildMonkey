        ...
        project.afterEvaluate {
            ...
            for(MavenizeTaskExtension task : config.mavenizeTasks) {
                ...
                project.task(name + "_$i") {
                    ...
                    if(task.useP2MirrorOutput) {
                        def p2Repo = "$project.eclipseMirror.target"
                        if(p2Repo != null && p2Repo != "null")
                            task.sourceP2Repository = p2Repo
                        // task should depend on publish p2 task then
                        it.dependsOn("mirrorP2Repository")
                    }
