...
class DownloadApplicationTask extends DefaultTask {
    ...
    private void downloadApplicationUnprotected(Project project) {
        ...
        project.logger.info("Download application from '${downloadUrl}' to '${targetFile.absolutePath}'")
        project.ant.get(src: new URL(downloadUrl), dest: targetFile)

        // extract it to the same location where it was extracted
        def targetFolder = new File(targetFile.parentFile, 'application')
        ...
        if (OperatingSystem.current().isWindows()) {
            project.ant.unzip(src: targetFile, dest: targetFolder, overwrite: true)
        } else {
            project.ant.untar(src: targetFile, dest: targetFolder, compression: "gzip", overwrite: true)
        }
    }
