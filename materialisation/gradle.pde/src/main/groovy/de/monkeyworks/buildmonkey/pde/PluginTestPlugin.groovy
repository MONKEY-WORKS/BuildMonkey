...
class PluginTestPlugin implements Plugin<Project> {
    ...
    static void addTaskCreatePluginTest(Project project) {
        ...
        //project.tasks.check.dependsOn pluginTest
    }
    ...
    static void installDepedenciesIntoTargetPlatform(Project project, Config config, File additionalPluginsDir, File testDistributionDir) {
        String names = prepareUpdateSite(project, additionalPluginsDir)
    ...
    private static Properties writeBundlesInfo(Project project, File testDistributionDir) {
        ...
        def startLevels = [ 'org.eclipse.equinox.common' : '2' , 
                            ...
                            'org.eclipse.osgi' : '-1']
/* Reduce bundles has no effect
        def whitelist = [   'org.eclipse.equinox.p2.metadata.repository',
...
//                                  if(artefact in needed || artefact in whitelist || !artefact.startsWith('org.eclipse')) {
    ...
    private static String prepareUpdateSite(Project project, File additionalPluginsDir) {
        ...
        def names = ""
        // Copy all bundles into the plugin folder
        bundles.each { bundleFile ->
            project.copy {
                from bundleFile
                into pluginFolder
            }
            names += ", " + transformBundleName("$bundleFile.name")
        }

        project.copy {
            from pluginFolder
            into pluginFolder
            rename '(.+)-(\\d+\\.\\d+\\.\\d+\\.jar)', '$1_$2'
        }
